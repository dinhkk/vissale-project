...
class FB
{
    ...
    public function fetchOrder($fb_page_id, $worker, $hostname)
    {
        ...
        if (! $this->config) {
            LoggerConfiguration::logError("Not found config for fb_page_id=$fb_page_id", __CLASS__, __FUNCTION__, __LINE__);
            return;
        }
        ...
        while (true) {
            ...
            foreach ($posts as $post) {
                ...
                $post_reply_by_scripting = isset($posts['reply_by_scripting']) ? unserialize($posts['reply_by_scripting']) : null;
                // xy lu tung post
                    ...
                    foreach ($comments as $comment) {
                        ...
                        if ($phone = $this->_includedPhone($message)) {
                            ...
                            if (! $parent_comment_id) {
                                $comment_reply = $this->replyCommentByScripting($message, $comment_time, $post_reply_by_scripting);
                                if (! $comment_reply) {
                ...
                $update_data['last_time_fetch_comment'] = $last_time_fetch_comment > $current_time ? $last_time_fetch_comment : $current_time;
    ...
    public function replyCommentByScripting(&$comment, &$comment_time, &$post_reply_by_scripting)
    {
        if ($comment_time && isset($this->config['reply_by_scripting']['out_of_work_time'])) {
